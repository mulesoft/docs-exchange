...
== Set up a cookie consent management for your domain

The General Data Protection Regulation (GDPR) stipulates that the owner of a web page must obtain and store the consents to the use of cookies from your visitors with a cookie management. We as information processors must offer a way for you to do the same in your vanity domain.

In order to correctly activate the cookie management feature in your vanity domain, you must contact your customer support and provide them the following information:

* Organization domain (e.g. `robo-cop`)
* Your registed domain (e.g. `developers.robocop.com`)

This data will be used by the customer support to create a `COOKIE_CONSENT_ID` for the recently registered vanity domain. The value of this variable will be provided by the customer support and cannot be transferred to other vanity domains.

=== Register Your Cookie Consent Id

Obtaining an access token as it was done in a previous step, now we must to register your `COOKIE_CONSENT_ID`:
...
----
If the request is resolved successfully, you should already have `Cookie Preferences` button visible in navbar page the next time you enter your vanity domain.

NOTE: The `COOKIE_CONSENT_ID` will be linked to your registered domain, so changing the domain will imply that you must repeat the process again.
