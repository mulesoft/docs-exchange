...
Exchange public portals that do not use vanity domains can use the xref:cookies.adoc[cookie consent manager] provided.

When you set up an Exchange public portal with a vanity domain, activate cookie consent management by contacting customer support with this information:

* Your organization domain (Example: `robo-cop`)
* Your registered domain (Example: `developers.robocop.com`)

Customer support provides you a `COOKIE_CONSENT_ID` for the recently registered vanity domain. This `COOKIE_CONSENT_ID` cannot be transferred to other vanity domains.

=== Register Your Cookie Consent ID

You must register your `COOKIE_CONSENT_ID`. Use the same token you obtained when you registered your domain.
