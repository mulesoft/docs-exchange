...
The following example publishes the API to an organization in the US control plane for Anypoint Platform. Defaults or environment variable settings are used for the unspecified options. 
...
Custom trigger conditions enable you to control whether APIs are published when the API Catalog `publish-assets` command runs. This example shows how `--trigger-criteria` options correlate with descriptor file trigger conditions to control whether or not to publish the described APIs.   
