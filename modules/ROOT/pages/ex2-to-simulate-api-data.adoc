...
+
image::exchange-rest-api-mocking-service.png["API Console showing the mocking service"]
...
+
image::exchange-patch-version-mocking.png["A table of patch versions for 1.0 with mocking instances"]
+
* Listed on the *API instances* page that also lists the mocking service URL.
+
image::ex2-api-instances.png["A table of API instances, their corresponding environments, URLs, and visibility"]
