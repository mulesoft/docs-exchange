...
In general, websites have required cookies necessary for the site to work correctly, functional cookies necessary for certain site functionality, functional cookies that are not necessary for any site functionality, and advertising cookies.

Exchange requires some cookies to work correctly, including cookies for session details and authentication information.

Exchange normally uses some functional cookies, including cookies for site analytics. Exchange does not require any functional cookies for site functionality.
...
|+__tld__+ |Session |Functional |Used to track visitors on multiple websites, in order to present relevant advertisement based on the visitor's preferences.
...
|_gid |1 day |Functional |Registers a unique ID that is used to generate statistical data on how the visitor uses the website.
|ajs%3Acookies |365 days |Functional |Supports use, service performance, and functionality of websites.
|ajs%3Atest |365 days |Functional |Counts visits and tracks other website traffic-related metrics for performance analytics.
...
|amplitude_id_7856322d613393b800727439f0954eb9mulesoft.com |3650 days |Functional |We use Amplitude to identify unique users visits in order to keep accurate web analytics.
|amplitude_idundefinedmulesoft.com |Session |Functional |We use Amplitude to identify unique users visits in order to keep accurate web analytics.
|amplitude_testmulesoft.com |Session |Functional |We use Amplitude to identify unique users visits in order to keep accurate web analytics.
...
|IDE |729 days |Functional |Supports the Doubleclick platform, a real time bidding advertising exchange.
...
|test_cookie |Session |Functional |Supports the Doubleclick platform, a real time bidding advertising exchange.
|XSRF-TOKEN |Session |Required |Helps prevent Cross-Site Request Forgery attacks for site security.
|zarget_visitor_info |365 days |Functional |Analytics and customization.
|===
