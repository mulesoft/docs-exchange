...
|tags |A string of comma-separated values to post as the asset's tags, for example, `tags=tag1,tag2`. If the tags field is not present or is empty in the request, the asset's tags remain unchanged. The string is the final version, so to delete one tag from an asset with two tags, send only the tag that will remain. |Optional
|fields.key |A string of comma-separated values to post as the asset's custom fields. Replace `key` with the key of the custom fields that you want to add or replace. The custom field must be configured in your organization. If the fields element is not present or is empty in the request, the asset's fields remain unchanged. The string is the final version, so to delete one custom field from an asset with two custom fields, send only the field that will remain. |Optional
|categories.key |A string of comma-separated values to post as the asset's categories. Replace `key` with the key of the category that you want to add or replace. The category must be configured in your organization. If the categories field is not present or is empty in the request, the asset's categories remain unchanged. The string is the final version, so to delete one category from an asset with two categories, send only the category that will remain. |Optional
...
This example publishes the category and custom field, which have the keys `categoryKey` and `fieldKey`. They must be configured for your organization. Publication fails if they are not configured for your organization or if they are not deleted from the pom.

It is not possible to submit new mutable data to an existing patch version such as 1.2.3. Instead, create a new patch version such as 1.2.4 with updated mutable data such as categories.

The documentation archive file sent as `files.docs.zip` contains each page as a Markdown file such as `Getting Started.md` in the root directory of the archive. Replace `Getting Started` with the title of the page. Include resources such as images in the optional subdirectory `resources`. Specify the page order with the property `pageOrder` on the root of the main object in the optional file `config.json`:
