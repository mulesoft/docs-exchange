...
|properties |Some asset types require additional properties specified with the properties prefix, such as `apiVersion`, `mainFile`, or `assetLink`. Example: `properties.apiVersion = v1` |
|===
...
Each asset is uniquely identified by the combination of its organization, group ID, asset ID, and version.

To specify the asset, replace the identifiers `:organizationId`, `:groupId`, `:assetId`, and `:version`.
