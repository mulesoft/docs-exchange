...
Asset icons are version agnostic. If an icon exists for a previous version of an asset, uploading a new icon for a subsequent asset version is ignored. The following example shows how to upload an asset icon. To specify the asset, replace the identifiers `:groupId`, `:assetId`, and `:version`. Ensure that the package format `files.icon.png` has the same file type as the icon file `my-asset-icon.png`.
