...
A timeout error can occur when publishing custom connector APIs when schema definitions import other schemas with external references. This happens if those references are either private or not reachable from the internet.
