= Publishing Assets Using the Exchange API
...
[NOTE]
====
The Exchange API is a REST API on version 2 (`/exchange/api/v2/`). The `/api/v3/` endpoints on `maven.anypoint.mulesoft.com` belong to a separate API, the Exchange Maven Facade API, which supports only Maven-based publishing. See xref:to-publish-assets-maven.adoc[] for Maven publishing.
====
...
. Log in to Anypoint Platform and download the https://exchange-files.anypoint.mulesoft.com/68ef9520-24e9-4cf2-b2f5-620025690913/689ce62f6cdfc382f974087bec28af21d3e9b2d525b74a3ab4a90b549e16eba7.zip?response-content-disposition=attachment%3B%20filename%3Dtraining-american-flights-api-1.0.0-raml.zip&Expires=1771053074&Key-Pair-Id=APKAJ3T3QD2CMFEXRAJA&Signature=irKCgALj61kdDfYr4ozcYuGf4B1kcFR0JgDAiVFxwDc8WBbkn-dQA1~zTJ2lJ7bBtzK7n6FbA3n3quc~zeuEeDu8BIPxt77B7x-nMS6LNVjLTbj2r89XJVbcbyjhVg0UyWQUUl-7u3YNw~V~pW-5KltWOaiHtwgQOLatVCQXNg0S2zetAXlxCA~fbM2YWiiTWgdWcu3CgbQuTzzFYfjlTJrbV~Hq8QHwXJpVivW3QaSYtJwhjgf7D3CaIePNrN9UwkQCWTyyta0Ne2x6ROBegLkjlGhL1RpmA3gYCuvV2VFezaWkjsMorEow6D5Ty~eafsDYF8k-EaxOO9xpNJtXXQ__[example file].
...
When publishing custom connectors, a timeout error can occur. This can happen if schema definitions import other schemas with external references are private or unreachable from the internet.

To resolve this issue, remove external references that are private or not reachable from the internet.
