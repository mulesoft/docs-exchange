...
Exchange assets are accessible from many places, such as Anypoint Studio (connectors, templates, and examples), API Manager (APIs, API groups, and policies), or Runtime Manager (applications). Agents, APIs, and MCP servers can be published from Anypoint Code Builder to Exchange.
...
== Agent Fabric Assets
Agent Fabric assets include agentic assets and tools to provide support and automate tasks. You can find agent, Large Language Model (LLM), and MCP server assets in the *Agents & Tools (New)* tab. 

You create agent network projects in Anypoint Code Builder which act as central hubs for defining, validating, and executing these multistep processes using agents and tools like LLMs and MCP servers. Anypoint Code Builder publishes agent network projects as assets to Exchange. Exchange tags them as brokers for identification. See the Anypoint Code Builder documenation. 

Agent Fabric assets follow the standard publishing workflow. See xref:workflow-sharing-assets.adoc[] for details.

See xref:agent-fabric::index.adoc[]for more information.
...
[[exchange-on-hyperforce]]
== Anypoint Exchange On Hyperforce
All features of Anypoint Exchange are supported on Hyperforce with these exceptions:

* xref:usage-and-engagement-metrics.adoc[Usage and engagement metrics] aren't supported.
* Amazon CloudFront isn't configured on Hyperforce. 
+
To download files from Japan or Canada Hyperforce environments, use these URLs:
