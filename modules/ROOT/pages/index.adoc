...
== Agent Fabric Assets
Agent Fabric assets include various assets to provide support and automate tasks. You can find agent, Large Language Model (LLM), and MCP server assets in the *Agents & Tools (New)* tab. 

You create agent network projects in Anypoint Code Builder which act as central hubs for defining, validating, and executing these multistep processes using agents and tools like LLMs and MCP servers. Anypoint Code Builder publishes agent network projects as assets to Exchange. Exchange tags them as brokers for identification. These assets follow the standard publishing workflow. See xref:workflow-sharing-assets.adoc[] for details.
