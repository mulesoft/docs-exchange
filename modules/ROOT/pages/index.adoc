...
Exchange assets are accessible from many places, such as Anypoint Studio (connectors, templates, and examples), API Manager (APIs, API groups, and policies), or Runtime Manager (applications). A2A agents, APIs, MCP Servers, and conductor domains, can be published from Anypoint Code Builder to Exchange.
...
The *Agentic* tab organizes assets such as A2A agents, conductor domains, and MCP servers.
