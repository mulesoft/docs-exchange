...
Anypoint Exchange is a curated catalog of reusable assets. Agent to Agent (A2A) agents, APIs, API groups, API spec fragments, conductor domains, custom assets, examples, GraphQL APIs, Media Control Protocol (MCP) servers, integration assets such as connectors, policies, RPA assets, rulesets, and templates are some of the types that are supported in Exchange. See xref:asset-types.adoc[Asset Types] for more information about each type of asset. You can catalog (publish), share, discover, learn about, and reuse assets within your organization of developers to facilitate collaboration, boost productivity, and promote standards.
...
Exchange assets are accessible from many places, such as Anypoint Studio (connectors, templates, and examples), API Manager (APIs, API groups, and policies), or Runtime Manager (applications). A2A agents, APIs, and MCP Servers can be published from Anypoint Code Builder to Exchange.
...
You can find A2A agents and MCP servers assets in *AI assets (Beta)* and *All assets*. Brokers groups created in Anypoint Code Builder are published to Exchange as A2A assets. Exchange automatically tags them as brokers for easy identification. 
