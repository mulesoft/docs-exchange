...
Use the help option with the commands to list the commands or command options and their descriptions:
...
After you have added the command to your shell profile, you can run it with options to control the results.  
...
Use the +api-catalog create-descriptor+ command to generate a descriptor file by discovering APIs in the designated folder structure.   
...
Use the dry-run parameter with the publish-assets command to generate a catalog.yaml file after autodiscovering all API candidates. xxx 
...
| -d, --dry-run=dry-run  | Runs the command to identify the APIs that are eligible to be published. No APIs will be published. The descriptor file specified in the catalog-file parameter defines the scope. |   
...
When you publish an API to exchange, the patch version is incremented by default. However, you can set the version using a combination of the API and asset version parameters as follows:
