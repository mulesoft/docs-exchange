...
| -t, --trigger=<descriptor-tag>:<value> --trigger=<descriptor-tag>:value  | Conditions that are applied to the descriptor file to determine whether the APIs described in the descriptor file should be published. To set multiple conditions, specify separate --trigger options for each condition.
...
The `--trigger` options work in conjunction with the `triggerConditions` section in the descriptor file, and all trigger conditions must be met for the APIs to be published. See <<Descriptor YAML Schema>>.  
