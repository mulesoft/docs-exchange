...
Before you run the commands to publish your APIs, you must create a descriptor file in each of the locations where you store your API projects. 

=== Create a Descriptor File

Run the +api-catalog create-descriptor+ command to identify APIs in a designated directory path and create a descriptor file with the information needed to publish these to Exchange. You can then specify the descriptor file in the +api-catalog publish-assets+ command to publish the APIs to Exchange. See <<Publish APIs Using CLI>>.
...
//==== Example 4: Do a dry run to ensure the descriptor file is valid
...
| tags  | A list of comma-separated text strings to describe the API. These appear on the asset's page in Exchange.| NULL. If not specified, no tags will be added.
...
Following is a descriptor file that describes the cataloging information for two APIs. 
