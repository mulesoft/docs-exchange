...
. Set required entitlements within the Anypoint Platform organization.
...
== Set Entitlements in Anypoint Platform

You must enable the API Catalog entitlement for the Anypoint Platform organization user. 

NOTE: This must be done by an Anypoint Platform user with administrative permission.

To enable the API Catalog entitlement:

. In Anypoint Platform, navigate to *Management Center > Admin*.
. Select *Home* in the left navigation.
. Search for and select the Anypoint organization for which you plan to use API Catalog CLI.
. In the organization's page, select *Settings* in the left navigation. 
. In the *API Manager and Others* section, select *API Catalog*.
. Click *Apply Changes*. 
...
==== Example 2: Generate a descriptor file by autodiscovering API candidates 

xxx

==== Example 3: Autodiscover API candidates and send the results to standard output
