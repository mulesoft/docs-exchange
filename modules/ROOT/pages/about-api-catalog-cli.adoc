...
* Images in documentation are resolved only if the reference is a full URL that points to an externally hosted image. 
* Links between documentation pages must use `../<key>` to refer to another Markdown file name `<key>.md` in the same directory. This doesn't resolve locally, but does in Exchange.
...
| -t, --trigger=<descriptor-tag>:<value> --trigger=<descriptor-tag>:value  | Conditions that are applied to the descriptor file to determine whether the APIs described in the descriptor file are published. To set multiple conditions, specify separate `--trigger` options for each condition. The `--trigger` options work in conjunction with the `triggerConditions` section in the descriptor file. All trigger conditions set in the descriptor file must be met for the APIs to be published.
...
The preceding example sets `triggerConditions` to enable the `api-catalog publish-asset` command with three `--trigger` options to publish APIs that: 

* Are in the branch `main` or any branches that have `release/` in the name.
* Have tags that match `support` or that have `release/` in the text.
* Are authored by the user `admin`. 

If none of the APIs in the descriptor file meet the conditions specified by the `--trigger` option, no APIs will be published. For more information, see the `--trigger` option for the <<api-catalog publish-asset, `api-catalog publish-asset` command>> and <<Publish an Asset Using Trigger Conditions>>.
