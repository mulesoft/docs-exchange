...
* Links to external resources are resolved only if the references are full URLs that point to externally hosted resources.
...
| -t, --trigger=<descriptor-tag>:<value> --trigger=<descriptor-tag>:value  | Values that are compared to trigger condition values in the descriptor file to determine whether the APIs described in the descriptor file are published. To set multiple conditions, specify separate `--trigger` options for each condition. The `--trigger` options work in conjunction with the `triggerConditions` section in the descriptor file.  For the APIs to be published, all trigger conditions set in the descriptor file must be matched by `--trigger` option values.
...
Following is an example `triggerConditions` section section in the `mydescriptor.yaml` descriptor file:
...
No APIS are published unless all trigger conditions set in the descriptor file are matched by `--trigger` option values. For more information, see the `--trigger` option for the <<api-catalog publish-asset, `api-catalog publish-asset` command>> and <<Publish an Asset Using Trigger Conditions>>.
