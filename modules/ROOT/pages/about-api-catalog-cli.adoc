...
. <<Create a Descriptor File,Create a descriptor file>> to identify the APIs to publish.
. <<Publish APIs Using CLI,Catalog an asset>> using the API Catalog CLI and view it in Anypoint Exchange.
...
Use the help argument with the commands to list the commands or command options and their descriptions. For example:
...
Before you run the commands to publish your APIs, you must create a descriptor file that has the information Exchange needs to catalog the APIs.  
...
You can use the `api-catalog publish-asset` command to publish your APIs to Exchange. You will typically embed this command in your automation tools, such as a CI/CD pipeline or custom scripts, to automatically trigger the cataloging of your API assets. The command publishes your APIs using the information specified in the `--descriptor-file` option.   
...
* Run the `api-catalog publish-asset` command. For example:
+
----
...
Following are example commands that show how several catalog options are used.
...
=== Configure Autocompletion in a Bash shell

To configure command autocompletion in a Bash shell, run `api-catalog autocomplete bash` and follow the instructions:
...
=== Configure Autocompletion in a Z Shell

To configure command autocompletion in a Z shell, run `api-catalog autocomplete zsh` and follow the instructions:
