...
TIP: If this is a test run, copy one or more APIs to local directory path.
...
Identify the directory path that contains the APIs you want to catalog. 
Run the +api-catalog create-descriptor+ command to identify APIs in a designated directory path and create a descriptor file with the information needed to publish these to Exchange. 

You can then specify the descriptor file in the +api-catalog publish-assets+ command to publish the APIs to Exchange. See <<Publish APIs Using CLI>>.
...
The name and location of the catalog descriptor file.  
  ...
  * If a valid YAML file exists, the command will catalog the assets as specified. | See <<Descriptor Files>>.

| -d, --dry-run=dry-run  | Runs the command to verify that the descriptor file is valid. No APIs will be published. |   --dry-run=descriptor
...
ANYPOINT_PASSWORD environment variable
 ...
 | Anypoint user password. Set the environment variable to avoid a prompt for the password.   | See <<Authentication>>.

| -s, --silent | Enable silent logging.|

| -t, --trigger=branch:master --trigger=tag:bla  | Filters that are applied to the descriptor file to determine whether an asset should be published. |  
...
                ]) { 
                    sh 'api-catalog-cli publish-asset -c $CATALOG_DESCRIPTOR -o $ANYPOINT_ORG_ID -a https://anypoint.mulesoft.com/ -b master'
                }
            }
        }
         stage('Deploy') {    
...
| triggerConditions | Section for information that will appear on the assets' pages in Exchange. This section includes branches, tags, and user. |

| branches | Specifies the branches from which to pull API information to be cataloged.  |
master

| tags  | A list of comma-separated text strings to describe the APIs to be published. These appear on the assets' pages in Exchange.| NULL. If not specified, no tags will be added.
...
| assetId | Specifies the ID of the asset to be published in Exchange. | By default, this ID is generated from the API specification *Title* field.  
...
Format: <Name of the Doc Page>:<File Location> | NULL. If not specified, no Doc pages will be imported. 
...
Format: <Category Name>: <Value(s)> | NULL. If not specified, no categories will be added.
...
Format: <Field>: <Value> | NULL. If not specified, no custom fields will be added.

| versioning | The asset version. By default the patch version is incremented by one. Example: ~1.x.x |  

| apiVersion | The API version. This is defined in the API specification. Example: v1 | 
