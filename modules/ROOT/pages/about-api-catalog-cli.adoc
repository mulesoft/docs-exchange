...
The preceding example sets `triggerConditions` to enable the `api-catalog publish-asset` command to publish the APIs described by the descriptor file if the command has three `--trigger` options with values that match the following: 
