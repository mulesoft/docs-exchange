...
This example shows a Jenkins script that runs the `api-catalog publish-assset` command.
...
This is defined in the API specification. 
...
Use the following notations to increase the asset version using the 
...
| '&'X.X.X | v4 | Increment to the next the major version. Because '&' is a reserved characater in YAML, it must be enclosed in a single quote. 
