...
| triggerConditions | This section enables you to set conditions to restrict the APIs that are included when the `api-catalog publish-asset` command is run with the `--trigger` option.   
...
The preceding example sets `triggerConditions` to enable the `api-catalog publish-asset` command with one or more `--trigger` options to publish APIs that: 
