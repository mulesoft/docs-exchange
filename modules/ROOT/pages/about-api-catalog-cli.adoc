...
NOTE: You can optionally configure the `api-catalog autocomplete` command so that you can use the tab key with a partially-typed command to autocomplete it. See <<Configure the Autocomplete Command>> for more details.
...
If you are using a Z shell (zsh), use the following command to add the autocomplete environment variable to your zsh script:
