...
Use the +api-catalog create-descriptor+ command to generate a descriptor file by discovering APIs in the designated API project's folder structure.   
...
This command parses the YAML schema that describes the APIs to catalog. The results are stored in a descriptor file that is then used by the +api-catalog publish-assets+ command to publish identified APIs to Exchange. See <<Publish APIs Using CLI>>.
