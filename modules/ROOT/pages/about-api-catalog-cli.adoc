...
NOTE: You can alternatively create a descriptor file manually instead of using the `api-catalog create-descriptor` command to generate the file. See <<Create a Descriptor File Manually>>.
...
* Create a descriptor file and copy it to the file path where your API projects are stored. See <<Create a Descriptor File Using the CLI>>.
...
==== Example 2: Perform a dry run to ensure that the descriptor file is valid
...
You can create a descriptor file manually rather than using the `api-catalog create-descriptor` command to create it. You can also update a descriptor file that you have already generated to ensure that it identifies the exact set of APIs that you want to publish.  
