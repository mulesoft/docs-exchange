...
The following example publishes the API to the organization `4c06ba7b-3ce8-425c-878c-b728dd7db60d` in the US Anypoint Platform control plane using the `./catalog.yaml` descriptor file in the main branch of the VCS repository:
...
--- 
 api-catalog publish-asset -a https://qax.anypoint.mulesoft.com -d catalog.yaml -o 1234abc-b34a-7cd3-4s32-12345abc2345 --client-id=1234567abcd2345gabc987656abc --client-secret --verbose >> mycatdryrun.log

---

==== Example 3: Publish an asset regardless of the content or metadata

If you want to publish a new version of the asset in Exchange regardless of the content, you should use the `--force-publish` option. If you want to publish a new version of the asset in Exchange regardless of the metadata content, you should use the `--force-update-metadata` option. 
...
---
 api-catalog publish-asset -a https://qax.anypoint.mulesoft.com --descriptor-file=mydescriptor.yaml -o 1234abc-b34a-7cd3-4s32-12345abc2345 --force-publish --force-update-metadata
...
---
 api-catalog publish-asset -a https://qax.anypoint.mulesoft.com -d mydescriptor.yaml -o 1234abc-b34a-7cd3-4s32-12345abc2345 -u myAnyPtAccount -p myPwd@4!myacct --trigger=ref:main --trigger=tag:migrate --trigger=status:complete
...
Use the following symbols before the version nodes to control the incrementation of an asset version:
...
The following table shows how the asset version is incremented for example semantic versioning strings. Because the API version must be explicitly specified in either the API specification or the descriptor file, it is not incremented or correlated to the asset version. 
...
| '&x.x.x' | v4 | Increment to the next the major version. 
