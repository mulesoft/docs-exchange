...
+
.. In Snowsight, click *Governance & security* and then click *Users & roles*.
...
.. In *Expires in*, set an expiry that follows your secret-rotation policy. The Snowflake maximum is 365 days.
.. For *Role*, select *One specific role*, and then select `ANYPOINT_SCANNER_ROLE`.
.. Click *Generate*, and copy the token immediately. Snowflake displays the token only once.
+
