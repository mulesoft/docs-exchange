...
The legacy search experience is also known as the original search. This search experience is available in Anypoint Platform Private Cloud Edition, MuleSoft Government Cloud, the US cloud, and the EU cloud. 

Discover assets by searching for specific assets from the landing page of the Exchange catalog. 

== Search by keyword 

Use the search field to search by keyword for example, enter a name of an API.

== Filter

You can filter search results by asset types for example, API groups, REST APIs, and rulesets by selecting an option from the *All types* dropdown menu. The options that appear depend on which types are available within an organization.
 
 
== Search in Organization

You can search only to the Exchange organization that you are currently viewing. If you are viewing a private Exchange organization, the search lists only that organization’s assets and not MuleSoft public assets.
