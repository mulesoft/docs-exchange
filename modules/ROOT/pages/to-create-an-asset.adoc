...
* You can also publish a RAML, OAS components, and JSON and RAML fragments using Design Center and publish the API to Exchange.
+
...
* API Spec Fragment:
+
RAML: Provide an API Fragment RAML specification file. Fragment specifications must be a RAML file (.raml). 
+
JSON: Provide an API Fragment JSON specification (.json) file. 
+
OpenAPI Specification (OAS): Provide an API Fragment OpenAPI specification file either JSON or YAML.
