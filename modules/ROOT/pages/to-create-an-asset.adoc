...
|===
| Asset Type | Creation Method

| OAS, RAML, RAML fragments, AsyncAPI, HTTP, WSDL, agents, LLMs, MCP servers, rulesets, custom
| Exchange *Publish new asset* dialog

| REST APIs, API spec fragments
| Anypoint Code Builder, Design Center, or the https://dev-portal.mulesoft.com/apis/exchange-experience.html#overview[Exchange Experience API^]

| RAML, OAS API specifications
| xref:runtime-manager::anypoint-platform-cli.adoc[Anypoint Platform CLI]. For example: `exchange asset upload --classifier raml --apiVersion v1 --name HelloWorld --mainFile helloworld.raml helloworld/1.0.0 /Users/nmouso/Downloads/helloworld.raml.zip`

| Examples, templates
| Mavenize feature in Anypoint Studio

| Connectors, policies, examples, templates
| xref:to-publish-assets-maven.adoc[Maven]

| API groups
| Anypoint API Manager

| Mule applications
| Anypoint Runtime Fabric (not visible in the Exchange UI)

| External libraries
| xref:to-publish-assets-maven.adoc[Exchange Maven Facade API] (not visible in the Exchange UI)
|===
...
* gRPC API: Provide a gRPC API specification file in a .proto file.
  ...
  "prompts": [
    {
      ...
      "arguments": [
        ...
        { "name": "tone", "description": "Desired tone (e.g., concise, friendly)", "required": false }
      ]
    }
  ]
}
