...
MCP server schema files are in JSON format. They define the definition of the tools, the data structures they use, and authentication. Upload the schema file to use when creating an MCP server asset or use the MCP server’s URL to fetch the schema information.
...
.. If required, enter the asset credentials.
...
. Select *Publish*.
. To create an instance of this asset, click *Instances* in the sidebar.
