...
To share an API, API Group, policy, example, template, or connector in either an Anypoint Exchange private instance or an Exchange public portal, create an asset of that type.
...
This illustration summarizes how some asset types (in green) appear in Exchange:
...
. (Optional) Select an option for *Lifecycle state*. The default is *Stable*.
. Select *Publish*.
