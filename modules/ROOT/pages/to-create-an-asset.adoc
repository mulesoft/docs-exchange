...
To share assets in either an Anypoint Exchange private instance or an Exchange public portal, create an asset an asset type.
...
. Select a method type from the list.
. Click *Choose file* to upload a .json file.
. Select a protocol.
. Click *Choose file* to upload a .json file for the agent.
////
...
LLMs are large language models that define model metadata, capabilities, parameters, and usage policies for discovery and governance across Exchange.

//LLM schema files are JSON format that the large language model should produce or consume.

. In Exchange, select *Publish new asset*.
. Enter a name for the asset.
. Select the *LLM* asset type from the dropdown list.
. Select a platform from the dropdown list.
//. Click *Choose a file* to upload a schema file in JSON format.
//. To edit the advanced settings, *GroupId*, *AssetId*, and *Version* (GAV), click *Advanced*. 
//+
//Exchange generates the group ID, asset ID, and version for you, and you can change these values as needed. Use the advanced settings to change the asset version.
//. Select an option for *Lifecycle state*. The default is *Stable*.
...
== Create a MCP Server
MCP serves provides the protocols and specifications for seamless integration between different AI systems and tools.

MCP server schema files must be in JSON format. They define the definition of the tools, the data structures they use, and authentication. When creating an MCP server asset, you upload the schema file you want to use.
...
. To create the file using the template, click *Download template* and enter the required information, then click *Choose file* to upload the file.
. To edit the advanced settings, *GroupId*, *AssetId*, and *Version* (GAV), click *Advanced*. 
+
Exchange generates the group ID, asset ID, and version for you, and you can change these values as needed. Use the advanced settings to change the asset version.
. Select an option for *Lifecycle state*. The default is *Stable*.
...
You can create assets in Anypoint Code Builder, API Designer, or Exchange. After the asset is created, the name, icon, and description properties can only be changed in the Exchange asset details page as described in xref:to-describe-an-asset.adoc[Describe an Asset].
