...
A GraphQL API an specification of a service that defines a schema, which is the collection of object definitions. These objects are used when interacting with the API, either as input data or part of the API responses. The GraphQL API allows you to query the API instead of accessing operation that returns a fixed definition of data.
...
. Select which GraphQL API schema to upload.
. To select the file to upload, click *Choose the file* .
