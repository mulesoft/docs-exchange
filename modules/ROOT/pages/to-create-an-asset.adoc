...
To share assets in either an Anypoint Exchange private instance or an Exchange public portal, create an asset type.
...
* You can also create REST APIs and API Spec Fragments using Anypoint Code Builder or Design Center and publish the API to Exchange.
+
...
== Create an Agent 
Agents are programs that perform tasks autonomously or semiautonomously. AI agents use the Agent2Agent (A2A) protocol to communicate and collaborate with each other to perform tasks.
...
If create an agent using *Other* as the protocol type, the file must be .json file. This card follows the structure of the A2A Spec without required properties.
...
To use an OpenAI LLM in your broker, you must first enable the OpenAI LLM service in your organization. Next, configure the LLM asset using one of these methods:

* Manual Creation: Create the LLM asset manually in Exchange, and then reference this asset in the YAML configuration file within Anypoint Code Builder.
* Automatic publishing by defining the LLM directly within the YAML configuration file in Anypoint Code Builder. The LLM asset is then automatically published to Exchange.

It's possible that you may need more than one one LLM asset in your organization.
...
The minimum required properties for an MCP server schema file is `transport` over SSE or HTTP. If you declare `capabilities`, you must have all four capability flags: `tools`, `resources`, `prompts`, and `logging`. If you provide `tools`, `resources`, or `prompts`, each must have name and identifier fields.
...
When an asset is published from Anypoint Code Builder to Exchange, the project name is used as the asset name in Exchange.
