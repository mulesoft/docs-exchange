...
+
An application and its API instance use the same client provider. If an organization administrator has configured xref:api-manager::configure-multiple-credential-providers.adoc[multiple client providers] in API Manager, then the application list shows only the applications that use the same client provider as the selected API instance.
...
* xref:api-manager::configure-multiple-credential-providers.adoc[Configuring Multiple Identity Providers for Client Management] in API Manager documentation
