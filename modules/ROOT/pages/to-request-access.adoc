...
+
image::ex2-edit-app2.png["The Create new application dialog showing fields to update for an OAuth authentication"]
+
Specify:
+
...
+
image::ex2-edit-app.png["The Edit application dialog showing fields to update for an external authentication"]
+
...
== Agent, LLM, and MCP Server Access
In Exchange, agents, LLMs, and MCP servers are assets with a portal. The *Instances* page on the portal lists all of the asset's instances.

When a developer requests access to these assets for a client application, the developer selects an instance, selects a client application, and selects a service-level agreement (SLA) if available.

. Select the asset from the Exchange catalog to view the details page.
. Click *Request access*.
. Choose the instance from the list.
. If you are using an existing application, choose from the list of applications.
+
An application and its instance use the same client provider. 
. Choose the service-level agreement (SLA) tier for the application if available.
. Click *Request access*.
. If the instance has no SLA tiers, a notification dialog shows that your request is received and approved and lists the client ID and client secret.
. If the instance has an SLA tier, a notification dialog shows that your request is received and pending. You'll be notified by email the status changes. You can also check the status on the application page.
== See Also
