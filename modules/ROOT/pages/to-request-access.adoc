...
+
An application and its API instance use the same client provider. If xref:api-manager::configure-multiple-credential-providers.adoc[multiple client providers] have been configured in API Manager, then the application list shows only the applications that use the same client provider as the selected API instance.
