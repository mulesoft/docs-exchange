...
Exchange administrators and organization owners create custom fields using the Exchange API to extend the Exchange asset metadata model and add custom data to any type of asset. You can refine a search using preset information by filtering assets by custom fields. The custom field filter is available only when assets in your organization have custom fields.  
