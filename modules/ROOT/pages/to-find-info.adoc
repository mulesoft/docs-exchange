...
Use the *Search assets* box to find assets by keyword. 

Agent network projects created in Anypoint Code Builder are published to Exchange as A2A assets. Exchange automatically tags them as brokers for easy identification. 
 

image:exchange-search-assets-box.png["The Search assets box"]
...
image:exchange-new-search.png["Search results with the keyword MuleSoft highlighted"]

For Agent Fabric, agent network projects created in Anypoint Code Builder are published to Exchange as agent assets. Exchange automatically tags these assets as brokers. 

To find agents that are brokers, search the Exchange catalog using the keyword `broker`.
...
* Type
+
Types can be agents, AsyncAPIs, connectors, LLM, MCP, REST APIS, rulesets, and more. See xref:asset-types.adoc[Asset Types] for more details.
...
If a filter has more than 50 values, the search results display the top 50, sorted in descending order by frequency. If you need to see more values, select the filter and enter a keyword in the search box. The search will then show the top 50 values that match your keyword, using the same sorting criteria.
...
image::ex2-saved-searches-groups.png["An example of two saved search groups"]
