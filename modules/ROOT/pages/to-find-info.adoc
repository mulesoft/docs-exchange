...
Exchange Search looks for terms in asset names, tags, categories, properties defined in API specifications, custom attributes (also known as custom fields), and the portal content shown on the asset page. Search includes RAML and OAS APIs and all other assets in Exchange.

Assets migrated from Exchange 1 also allow the search to include text in an asset's description.
...
If you search in a private Exchange, the search only lists the private Exchange assets and not public Exchange assets.

If a search matches some versions of an asset and not other versions of the asset, then it returns the asset's latest matching version, even if that is not the asset's latest version.
...
Exchange searches API specification properties from the JSON LD provided by AMF.
...
Exchange searches data from these node types and properties in the JSON LD:

[%header%autowidth.spread]
|===
|Type |Property
|http:EndPoint |http:path
|http:EndPoint |schema-org:name
|http:EndPoint |schema-org:description
|hydra:Operation |hydra:method
|hydra:Operation |schema-org:description
|http:Parameter |schema-org:name
|http:Parameter |schema-org:description
