...
Custom fields are created by Exchange administrators and organization owners using the Exchange API to extend the Exchange asset metadata model and add custom data to any type of asset. The custom field filter lets you refine a search using preset information. The filter is available only when you have assets in your organization with custom fields.  
...
|===
|Data Type |Operator
|Enum|Contains and not contains. The default value is contains.
|Number|Equals, not equals to, greater than, less than, greater than or equal to, and less than or equal to. The default value is equals.
|Date  |Equals, not equals to, greater than, less than, greater than or equal to, and less than or equal to. The default value is equals.
|Text |Equals and not equals to. The default value is equals.
|List of numbers |Contains and not contains. The default value is contains.
|List of strings |Contains and not contains. The default value is contains.
|===
...
The following example shows the search filter with two custom fields. You can add as many custom fields as you need:

image:ex-custom-field-operator-value.png
