= Change Version of an API Asset
...
If the Group ID and Asset ID match an existing asset of the same type, a new Asset Version is created. The API Version can be different than the existing one.
...
Not all changes require a new version. APIs designed for backward compatibility do 
normally not require new versions for minor changes and is a recommended practice 
to design for this. Nevertheless, it is not always possible to design APIs in 
...
|===
|Type of Change |Requires a New Version |New Version is Not Required
| Service contract |
...
* Minor versions: When introducing a change to the API that does not require an API user to change, or when fixing a bug.
