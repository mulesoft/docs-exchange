...
Because an HTTP API asset is an endpoint, to which you can provide only policies or proxies, the *API version* field doesn't apply. HTTP APIs have only major version increases and do not have minor or patch version changes.

The *Version* and *API version* fields are visible only when you add a new asset or when you add a new version to an existing asset.
...
Only change the API version when an updated API specification has changes that are not backwards compatible from the new version to the old version.
...
* Quality of service (QoS) -- When a change affects the quality of service, such as response time and availability, the major version and API version may need to change as the QoS can break applications that use the API.
...
* Major versions: When introducing a change in the structure of the API that requires the user of the API to adapt the interface on the consumer side, such as a new required operation argument.
* Minor versions: When introducing a backward-compatible change to the API that does not require an API user to change, such as a new optional element.
