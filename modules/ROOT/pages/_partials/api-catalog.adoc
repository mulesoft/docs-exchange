...
|  --external | Generates an `exchange.json` file for each API described in the descriptor file and adds a reference to each of those in the generated descriptor file using the `ref` option.
|===
