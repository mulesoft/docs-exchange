...
<<<<<<< Updated upstream
The workflow to create custom policies for Mule 3.9 and Mule 4 in Anypoint API Manager has not changed. Continue to use the process of developing the policy, packaging the custom policy, uploading it to Exchange, and applying the policy to an API through Anypoint API Manager as described in the Anypoint API Manager documentation. Creating custom policies for Anypoint Flex Gateway and Mule 4 runtime is supported in Exchange.

The following information describes the workflow for publishing custom policies for Anypoint Flex Gateway and Mule 4:
=======
The current workflow to create custom policies for Mule 4.0 (or is it for 3.9 and earlier) has not changed. Continue to use the process of developing the policy, packaging the custom policy, uploading it to Exchange, and applying the policy to an API through Anypoint API Manager as described in the Anypoint API Manager documentation.

The following information describes the workflow for publishing custom policies for Anypoint Flex Gateway:
>>>>>>> Stashed changes
...
Before you can publish a custom policy for Flex Gateway or Mule 4, ensure you have:

. Administrator or contributor permission
. Created policy definition files 
. Created implementation files

<<<<<<< Updated upstream
== Create Policy Definition Files
Policy definition files must be created before you can upload the policy implementation. The definition files that are required to, a JSON and a YAML configuration file must be created. You can use any text editor tool. (This section must describe how to create the JSON file and the YAML file with examples).

A JSON file contains the implementation of the policy using JSON schema language.
...
A YAML file defines the configurable parameters of the policy. Anypoint API Manager uses this file to render the UI to display user inputs for the policy.
...
A YAML file defines the configurable parameters of the policy. Anypoint API Manager uses this file to render the UI to display inputs for the policy.
>>>>>>> Stashed changes

(TO DO add guidelines of how to write the JSON and YAML files and include examples. Get examples from Mariano and Philip)
...
=======
After creating the policy configuration files (JSON and YAML), create the policy asset in Exchange.
>>>>>>> Stashed changes
...
=======
Add the implementation files for the policy to be available in runtime. The required implementation files are a binary file such as a WebAssembly file for Flex Gateway or JAR for Mule 4 runtime and a YAML metadata file. 
>>>>>>> Stashed changes
...
. Click Close.
=======
You can add and remove a version for the implementation from the (name of page) page.
. To add a new implementation version, click the three vertical dots icon and select Add new version.
. Enter the new version (need to see steps)?
. To delete a version, a new implementation version, click the three vertical dots icon and select Delete Version. 
Click Close.
