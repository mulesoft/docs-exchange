...
Add a scanner to discover, import, and sync MCP servers from Amazon Bedrock AgentCore MCP Gateway into Exchange and to discover and read policies from MCP server configurations. Then you can govern the MCP servers and consume them in other applications.
...
* An IAM user with an inline policy that allows these read/runtime actions:
+
