...
When deleting assets, admins determine if the GAV (group ID, asset ID, and version) information of the deleted asset can be reused for a different asset. Therefore, when you delete an asset, you might not be able to reuse its GAV for new asset.
...
+
If GAV reuse is enabled, you have the option to reuse the GAV for another asset. If you don't select this option, the asset version's GAV will be deleted and can't be reused for another asset. If you select this option, the asset version's GAV can be reused.
. Click *Delete asset*.
