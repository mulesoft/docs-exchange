...
Enable TLS support for your generated connector by adding a `properties.json` file to the root directory of your Design Center API project. 
...
If you add an empty `properties.json` to your API project, or don't add it at all, the Rest Connect Service generates the standard artifacts without TLS support.  

It is best to add the `properties.json` file only if you want to enable TLS support, or explicitly opt out of it. 
