...
* TLS is enabled, so that Anypoint Studio displays a form for collecting the connector's required TLS connection configuration information. You can view this form from the *General* tab. To see the connection configuration information, select either *Edit inline* or *Global reference* from the *Tls context* field.
+
image::rest-connect-tls-configuration.png[, 50%, 50%]
