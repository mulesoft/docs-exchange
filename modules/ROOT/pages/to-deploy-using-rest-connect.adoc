...
* TLS is enabled, so that Anypoint Studio displays a form for collecting the connector's required TLS connection configuration information. This can be found under the connector's configuration under "Tls context" as shown here:

image::rest-connect-tls-configuration.png[The tls context will show its properties after selecting either "Edit inline" or "Global reference", 50%, 50%]
