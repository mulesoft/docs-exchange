...
Mule 3 connectors download as a ZIP files, and Mule 4 connectors download as JAR files.
...
To modify a generated connector name, point to the REST Connect library and use the `operationName` annotation from a method such as GET, POST, and DELETE, for example:
...
You can also change the text in `displayName`, for example:
...
Add a `properties.json` file to the root directory of your Design Center API project to enable TLS support to enhance security for your app. 
...
+
Adding this property and setting it to true customizes the connector generation so that when the Rest Connect Service receives the property, the connector is upgraded to run on Mule runtime 4.6. and the `tlsEnabled` flag propagates to the connector generation. 

This enables Anypoint Studio to display the appropriate form to collect the information required for TLS connection configuration. 
 
It is best to add the `properties.json` file only if you want to enable TLS support, or explicitly opt out of it. If you don't add a `properties.json` file to your API project, the REST Connect Service generates the standard connector artifacts, without TLS support.

When you add a properties file to your project, ensure that the `tlsEnabled` property is correctly set, with a valid boolean value. If the `properties.json` file is invalid or if the property is incorrectly set, the connector generation for both Mule 3 (DevKit) and Mule 4 (SmartConnector) versions is aborted.
