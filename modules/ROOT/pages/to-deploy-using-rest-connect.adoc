...
REST Connect supports only one input/output type and uses the first one declared in the API
specification. You can change this behavior by using the default property from the REST Connect library.
...
+
image::rest-connect-tls-configuration.png["The form for TLS connection configuration information", 50%, 50%]
...
+
image::rest-connect-ready-to-deploy.png["Copy link button in the menu"]
+
. After you get your external callback URL, specify the same URL in your GitHub settings.
+
image::rest-connect-auth-callback-url.png["Authorization callback URL field in GitHub with the specified external callback URL value"]
+
...
* Chinese characters are not supported in API specs or their dependencies. Japanese characters are supported in the `description` and `example` tags.
