...
* TLS is enabled, so that Anypoint Studio displays a form for collecting the connector's required TLS connection configuration information. To see the connection configuration properties, select either *Edit inline* or *Global reference* from the *Tls context* field.
+
image::rest-connect-tls-configuration.png[Form for collecting configuration information, 50%, 50%]
