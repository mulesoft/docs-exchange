...
+
. In your project's POM file, set the asset type.
+
For example:
+
[source,xml,linenums]
...
mvn deploy
----
. To share the location of an asset, use this URL syntax:
+
----
https://anypoint.mulesoft.com/exchange/api/v3/assets/GROUP_ID/ASSET_ID
----
+
Replace GROUP_ID with the asset's group ID and replace ASSET_ID with the asset ID. In most cases the organization ID is the same as the group ID.
+
In the EU, use:
+
----
https://eu1.anypoint.mulesoft.com/exchange/api/v3/assets/GROUP_ID/ASSET_ID
----
...
When you use Maven Facade to publish connectors, policies, custom assets, examples, and templates, a documentation portal is automatically created for them in Exchange and you can xref:to-describe-an-asset.adoc[document them normally].
...
----
  <repositories>
    <repository>
      <id>Repository</id>
      <name>Corporate Repository</name>
      <url>https://maven.eu1.anypoint.mulesoft.com/api/v3/organizations/ORG_ID/maven</url>
      <layout>default</layout>
    </repository>
  </repositories>
----
