...
The Exchange Maven Facade API does not support dynamic parameters for fields other than `<artifactId>` and `<version>`. If the uploaded asset has a defined parent, the parent must be a valid and already uploaded Exchange asset for the dynamic parameters to be properly interpreted.

New assets count toward your organization's xref:asset-versions.adoc#asset-limits[asset limit].

[NOTE]
====
The v3 in this page refers only to the Exchange *Maven Facade API*. If you’re publishing assets over REST using the Exchange API, that API remains v2. Use `/api/v3/` endpoints only when publishing with the Mule Maven plugin or the `exchange-mule-maven-plugin`. See xref:exchange-api.adoc[] for REST-based publishing.
====
...
[IMPORTANT]
====
Starting August 1, 2026, publishing custom connectors and Mule plugins to Exchange requires Java compatibility metadata in `mule-artifact.json` for new versions that change supported Java compatibility.
...
Use Maven 3.8.0 or later (3.9.8+ recommended) and JDK 17+ in your build pipeline.

This change doesn't affect connectors already published to Exchange or running applications.

For complete requirements and update instructions, see xref:mule-sdk::publishing-connectors-exchange.adoc[].
====
...
+
Don't specify the `<type>` element. In the `mule-maven-plugin` configuration, set `<classifier>` as `mule-application` (default), `mule-application-template`, `mule-application-example`, or `mule-policy`.
+
Mule 4 applications aren't displayed in the Exchange asset list.
+
...
=== Resolve Parallel File Publishing Failures with Maven 3.9.12 or Later

Maven 3.9.12 and later versions enable parallel file publishing by default. The Exchange Maven Facade API v1 and v2 require sequential file publishing and do not support this behavior. If you publish to a v1 or v2 endpoint using Maven 3.9.12 or later, the publish operation fails.

To resolve this issue, use one of these workarounds:

* *Upgrade to the Maven Facade API v3* — Use a v3 endpoint instead of v1 or v2.
* *Upgrade to ASF Parent POM 37* — Use https://maven.apache.org/pom/asf/[ASF Parent POM 37^], which includes the workaround for parallel publishing.
* *Disable parallel publishing from the command line* — Add the following property to your Maven command:
+
[source,console]
----
-Daether.connector.basic.parallelPut=false
----
* *Disable parallel publishing in your project* — Create a `.mvn/maven.config` file in your project root directory and add the following line. See https://maven.apache.org/configure.html#mvn-maven-config-file[Maven configuration^] for more information.
+
[source,console]
----
-Daether.connector.basic.parallelPut=false
----
* *Disable parallel publishing globally* — Add the following property to an always-enabled profile in your `~/.m2/settings.xml` file:
+
[source,xml]
----
<profiles>
  <profile>
    <id>disable-parallel-put</id>
    <properties>
...
<activeProfiles>
  <activeProfile>disable-parallel-put</activeProfile>
</activeProfiles>
----
...
The GitHub repository of Exchange documentation examples includes a complete example of creating a https://github.com/mulesoft/exchange-documentation-samples-labs/tree/master/template-with-mutable-data[Mule application template with documentation and tags, role=external, window=blank].
...
Publish custom assets with the `exchange-mule-maven-plugin` plugin. The latest version of this plugin is 0.1.7.
...
Parent POMs are published to Exchange as custom assets. See the previous section to publish a custom asset.
If you want to publish a maven project, like a mule application, using the Mule Maven Plugin, and your maven project depends on a Parent Pom that has already included the exchange-mule-maven-plugin, you need to https://maven.apache.org/guides/mini/guide-configuring-plugins.html#Using_the_.3Cinherited.3E_Tag_In_Build_Plugins[prevent the inheritance]:
