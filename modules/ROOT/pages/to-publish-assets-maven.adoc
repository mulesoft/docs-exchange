...
Your POM file must include the required plugin +org.mule.tools.maven.mule-maven-plugin+ which is not compatible with +org.apache.maven.plugins.maven-deploy-plugin+. Make sure you are not including the latter as well in your POM or settings files.
