...
** The `name` property represents the visible name of the asset. It only be used when the asset has the first version of a combination of a GroupId and an artifactId. To change it, go to the asset portal: see xref:to-describe-an-asset.adoc[To Describe an Asset] for details.
