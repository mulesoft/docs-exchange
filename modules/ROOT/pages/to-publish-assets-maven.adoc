...
Authentication works using a Connected Application.
In order to use it a Basic authentication should be provided where
* the username should be defined as: `~~~Client~~~`
* the secret should be defined as: `clientId~?~clientSecret`

Also, the connected application could be included in your repository into the settings.xml file in a similar way:
