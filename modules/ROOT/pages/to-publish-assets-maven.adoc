...
Dynamic parameters for fields other than <name> and <description> are not supported. If the uploaded asset has a defined parent, the parent must be a valid and an already uploaded Exchange asset for the dynamic parameters to be properly interpreted.
