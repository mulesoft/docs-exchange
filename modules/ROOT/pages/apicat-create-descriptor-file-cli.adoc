...
* File with `ref` options: The command gives a parsing error, as the `update-descriptor` command does not support descriptor files with references to external `exchange.json` files.
