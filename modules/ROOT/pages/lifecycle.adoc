...
When you create an asset in the Exchange user interface, select the `development` state or the default `stable` state.

== Search Using the Graph API by Lifecycle State

To consume an asset and its lifecycle state, execute this command:
...
To search for an asset by lifecycle state, replace `[development, published, deprecated]` with your list of desired lifecycle states and execute this command:
...
== Possible Future Lifecycle State Features

The general availability (GA) release of lifecycle states will possibly include additional features that are not included in the beta release:

* Audit log integration to track asset state promotions
* Preventing invalid lifecycle state transitions in the user interface
* Filtering assets in the home page search by lifecycle state
* Showing asset lifecycle state in search results
* Changing version-specific state in the versions table
