...
.. Click the three dots icon, select *Search for content*, and enter a keyword in the box.
.. Use the *CTRL+F* or *Command+F* keyboard shortcut and enter a keyword in the *Search content* box.

== Run a GraphQL Query
Use the query editor to write queries in the console and run them in real-time. You can use autocomplete or *Control+Space* to add fields without knowing the specific syntax. The query editor is located in the *Query* panel. 
...
. Click in the editor and start typing your query using autocomplete or press *Control+Space*.
...
. From the *Instance selector*, select an available server instance to use to run the query. If there are no server instances available, the *Execute* button is disabled.
. To add a new a non-managed instance to run the query, click *Add new instance*.
