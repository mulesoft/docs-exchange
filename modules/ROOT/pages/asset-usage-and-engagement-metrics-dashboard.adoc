...
The dashboard displays the metrics for the top fifty assets for a period of the last 7 days, 30 days, or 60 days that are ranked by the most consumption events. You can filter the top assets by API specification fragments and REST API types and by one or more organizations. Export metric data into a CSV file by clicking CSV to Export.
