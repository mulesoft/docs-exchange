...
NOTE: To see updated permissions, users must log out of Exchange and log in again.
...
For connected apps, the experience differs. If an asset is created using a connected app, the Asset Administrator role is granted to the connected app owner. You can’t use connected apps with the Exchange Creator permission to create new versions of existing assets.
...
== Remove Project Administrator Role

Remove the Project Administrator role when you reach the limit. Project Administrator is an internal role included in the Exchange Contributor permission.
...
The `identity_ids` value is the user ID. The remaining values identify the role to remove. To identify the role data, retrieve existing roles using this request:

[source,console]
----
curl 'https://anypoint.mulesoft.com/accounts/api/users/:userId/roles?role_groups=true&teams=true' \
  -H 'Authorization: bearer YOUR_TOKEN'
----

== Remove Roles When Limits Are Reached

One can remove their roles using the request below. This is useful when the role limit has been reached.
----
DELETE '/accounts/api/organizations/:organizationId/roles?role_groups=true&teams=true'
-H 'Authorization: ***
-H 'Content-Type: application/json'
-d '{
           "assign": [],
...
The values used in the body refer to the role one wants removed. In order to know this data we can learn what roles we have using the following request:
----
GET '/accounts/api/users/:userId/roles?role_groups=true&teams=true'
-H 'Authorization: ••••••'
----
