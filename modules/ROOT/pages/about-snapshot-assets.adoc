...
To create an snapshot asset you only have to add the `-SNAPSHOT` sufix to the asset version. For example: `1.0.0-SNAPSHOT`.
...
To republish an snapshot is not necessary to do any extra thing, publishing in the same way you did the fist time will be enough.


== What happens when publishing a snapshot asset version ?

Some things you should take in mind when you create an snapshot version.

* Although a snapshot version can be listed in the Exchange private portal, it can not be listed in the public portal. So, snapshot asset versions wont be showed in the public portal.
...
* When asking for assets through Exchange graph service, it don't retrieve the snapshot versions as default. To include them it is needed to add the property: `includeSnapshots: true`. To find more information, see `See Also` section.
...
* If a new file has been created as part of the snapshot version, this one will be the only file of the version and the files of the previuos snapshot version won't exist anymore.
