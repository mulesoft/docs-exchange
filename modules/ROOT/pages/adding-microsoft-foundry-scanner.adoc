...
+
** The application principal from the prerequisite step must be assigned the Cognitive Services User role within the Azure portal to grant the scanner access.
...
|*Project Endpoints* |Enter the project endpoint URLs separated by commas. Confirm you are using the public endpoint, not a private or VNet-restricted endpoint. If the connection fails, verify your endpoint type matches your network configuration, as private endpoints may not be accessible to the scanner running on AWS infrastructure.
|===
