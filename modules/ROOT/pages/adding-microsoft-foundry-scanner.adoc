...
|*Project Endpoints* |Enter the project endpoint URLs separated by commas. The endpoints must be publicly accessible; private endpoints with network restrictions will fail because the scanner runs on AWS infrastructure with no private connectivity to customer Azure environments.
|===
