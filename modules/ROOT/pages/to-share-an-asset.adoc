...
If a connector is shared with an external organization, then a user in that organization can use Studio or Flow Designer to search for the connector, configure it, and use it in a Mule application.

If an example or template is shared with an external organization, then a user in that organization can use Studio or Flow Designer to implement a Mule application based on the shared template.
