...
For an example publish command using the `--trigger-criteria` flag, see 
...
* If a `--version-strategy-criteria` flag set in the publish command 
...
For an example publish command using the `--version-strategy-criteria` flag, see 
...
ref:: As an alternative to describing the APIs in your project using the preceding options, you can use the `ref` option to reference `exchange.json` files for each API as shown in the following example:
+
----
projects:
