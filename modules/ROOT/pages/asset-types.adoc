...
* GraphQL API 
+
An specification or description of a service like RAML and OAS. It defines a schema, which is the collection of object definitions. These objects are used when interacting with the API, either as input data or part of the API responses. GraphQL enables you to query the API instead of accessing operation that returns a fixed definition of data. 
* HTTP APIs
+
