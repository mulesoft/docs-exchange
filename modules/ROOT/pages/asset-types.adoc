...
+
Activity templates are reusable code within RPA to standardize most used activity steps and make them available to other developers in the organization. Binary file type is .calw.

** RPA Process templates
+
Templates for RPA processes that can be imported into RPA Builder as projects and can be further customized before deploying to RPA bots. Process templates are built on common patterns for RPA to build automations faster. Binary file type is .crpa.
