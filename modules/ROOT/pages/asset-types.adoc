...
* API Spec Fragments
+
A RAML document that has a version and an identifier but is not in itself a complete RAML specification. API spec fragments are also known as _RAML fragments_. Exchange supports JSON, RAML, or OpenAPI Specification (OAS) files.
