...
For published assets with instances not managed by API Manager, asset administrators can add, edit, and delete non-managed instances that are serving AsyncAPIs, GraphQL and REST APIs. The name, URL, and visibility of a non-managed instance can be changed.

. Select the asset.
. In the navigation bar, select *API Instances*.
. Click *Add new instance*.
. In the *Non-managed instances* section, enter the instance name, URL, and select a visibility option and click image:ex-checkmark-icon.png["checkmark icon",3%,3%].
...
. To delete a non-managed instance, click image:ex-trash-can-icon.png["trash can icon",3%,3%] and click *Delete instance*.
